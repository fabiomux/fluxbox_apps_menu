...
module FluxboxAppsMenu
  ...
  class CLI < Thor
    ...
    method_option :filename, aliases: "-f", default: "",
                             desc: "Save as filename."
    method_option :verbose, aliases: "-v", default: false, type: :boolean,
                            desc: "Print helping output messages."
    method_option :overwrite, aliases: "-o", default: true, type: :boolean,
                              desc: "Overwrite the app menu if exists."
    def build
      FluxboxAppsMenu::Builder.new(options).create_menu
    end

    desc "config", "Make a local copy of the configuration file."
    method_option :overwrite, aliases: "-o", default: false, type: :boolean,
                              desc: "Overwrite the config file if exists."
    def config
