...
module FluxboxAppsMenu
  ...
  #
  # No categories for the menu item error.
  class NoMenuCategories < StandardError
    def initialize(item)
      super "Error! ".bold.red + "The menu item \"#{item}\" doesn't have any category, \
        fix it to \"fluxbox_apps_menu.yaml\""
    end
  end

  #
  # No mapped categories error.
  #
  class NoMappedCategories < StandardError
    def initialize(params)
      super "Error! ".bold.red +
        "The item \"#{params[:name]}\" doesn't have any mapped category among #{params[:categories].join(", ")},\
         fix it to \"fluxbox_apps_menu.yaml\""
    end
  end
